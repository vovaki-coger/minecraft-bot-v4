      ...
      // Lerp по 25°/тик = максимум быстрого живого игрока.
      {
        ...
        bot.lookAt = async function(point, force = false) {
          ...
          try {
            ...
            const totalAngle = Math.max(Math.abs(dYaw), Math.abs(dPitch));
            const steps = Math.ceil(totalAngle / MAX_DEG);
            // Lerp только для больших поворотов >90°. Маленькие — мгновенно,
            // иначе бот бьёт/рубит спиной пока ждёт завершения анимации.
            if (steps <= 1 || force || totalAngle < Math.PI / 2) return _origLookAt(point, force);
