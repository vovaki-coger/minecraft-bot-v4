...
export default function FarmTab() {
  ...
  async function handleStart() {
    ...
    const args = mode === "trees"
      ...
      : { radius, crop: cropType, useBoneMeal, delay: delayMs };
  ...
  const cropColor = mode === "crops" ? (CROP_TYPES.find(c => c.id === cropType)?.color || "#ffd54f") : modeColor;
      ...
      <div style={{ flex:1, overflowY:"auto", padding:8, display:"flex", flexDirection:"column", gap:7 }}>
            ...
            <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:4 }}>
              {CROP_TYPES.map(c => (
                <button key={c.id} onClick={() => setCropType(c.id)} style={{
                  padding:"6px 8px", fontSize:11, fontFamily:"monospace", borderRadius:4, cursor:"pointer",
                  background: cropType===c.id ? `rgba(${c.color.replace('#','').match(/../g)?.map(h=>parseInt(h,16)).join(',')},0.08)` : "rgba(0,0,0,.3)",
        ...
        <div style={{ background:"rgba(0,0,0,.3)", border:"1px solid #1a2040", borderRadius:5, padding:8 }}>
          <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center", marginBottom:6 }}>
            ...
            <span style={{ fontSize:13, color:modeColor, fontFamily:"monospace", fontWeight:"bold" }}>{radius}м</span>
          </div>
          <input type="range" min={5} max={60} step={5} value={radius} onChange={e => setRadius(Number(e.target.value))}
            disabled={isRunning} style={{ width:"100%", accentColor:modeColor, cursor: isRunning ? "not-allowed" : "pointer" }} />
        ...
        <div style={{ background:`rgba(${mode==="trees"?"126,204,73":"255,213,79"},.04)`, border:`1px solid rgba(${mode==="trees"?"126,204,73":"255,213,79"},.15)`, borderRadius:5, padding:"7px 10px" }}>
          <p style={{ fontSize:10, fontFamily:"monospace", color:"#3a5070" }}>
            Задача:&nbsp;
            <span style={{ color: mode==="crops" ? cropColor : modeColor }}>
              {mode==="trees"
          ...
          {mode==="trees" && (
            <p style={{ fontSize:9, color:"#2a3a5a", marginTop:3 }}>
              Рубит → сажает саженцы → бонемил → повторяет
            </p>
          )}
        </div>
        ...
        <button onClick={isRunning ? handleStop : handleStart} disabled={!isOnline} style={{
          ...
          background: isRunning ? "rgba(255,34,85,.08)" : isOnline ? `rgba(${mode==="trees"?"126,204,73":"255,213,79"},.08)` : "rgba(0,0,0,.3)",
          border: isRunning ? "1px solid rgba(255,34,85,.4)" : isOnline ? `1px solid rgba(${mode==="trees"?"126,204,73":"255,213,79"},.4)` : "1px solid #1a2040",
            ...
            <div style={{ maxHeight:160, overflowY:"auto", padding:6 }}>
              {log.map((entry, i) => {
                ...
                const col = entry.msg.startsWith("⏹") ? "#ff5555"
                          : entry.msg.startsWith("▶") || entry.msg.startsWith("🚀") || entry.msg.startsWith("✅") ? modeColor
