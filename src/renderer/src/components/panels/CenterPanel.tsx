    ...
    <div className="panel" style={{ flex: 1, display: "flex", flexDirection: "column", overflow: "hidden" }}>
      ...
      <div style={{ flex: 1, overflowY: "auto", padding: 8, display: "flex", flexDirection: "column", gap: 7 }}>
        ...
        <div style={{ display: "flex", gap: 5, flexWrap: "wrap" }}>
          <button
            className={`btn ${(isOnline || bot.status === "connecting") ? "btn-danger" : "btn-primary"}`}
            onClick={handleConnect}
            disabled={loading === "conn" || bot.status === "connecting"}
            style={{ flex: 1, minWidth: 110, fontSize: 11 }}
          >
            {bot.status === "connecting" ? "⏳ Подключение…" : isOnline ? "⏹ Отключиться" : "▶ Подключиться"}
          </button>
          <button
            className="btn" onClick={handleToggleAI}
            style={{
              fontSize: 11,
              borderColor: bot.config.aiEnabled ? "rgba(157,96,255,.45)" : "#1a2040",
              color:       bot.config.aiEnabled ? "#9d60ff" : "#2a3a5a",
              boxShadow:   bot.config.aiEnabled ? "0 0 8px rgba(157,96,255,.18)" : "none",
            }}
          >⚡ ИИ: {bot.config.aiEnabled ? "Вкл" : "Выкл"}</button>
          <button
            className="btn" onClick={handleSurvivor}
            disabled={!isOnline || loading === "surv"}
            style={{
              fontSize: 11,
              borderColor: isSurv ? "rgba(255,34,85,.45)" : "#1a2040",
              color:       isSurv ? "#ff2255" : "#2a3a5a",
              boxShadow:   isSurv ? "0 0 8px rgba(255,34,85,.18)" : "none",
              opacity: !isOnline ? 0.35 : 1,
            }}
          >
            {loading === "surv" ? "⏳…" : isSurv ? "⏹ Выживальщик" : "⚔️ Выживальщик"}
          </button>
        </div>
