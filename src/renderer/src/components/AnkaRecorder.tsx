...
const slotStyle = (
  ...
  width: SLOT_SIZE,
  height: SLOT_SIZE,
  background: lmbFlash ? "#1a4a1a" : rmbFlash ? "#4a3a0a" : isHotbar ? "#1a1a2a" : "#181818",
  border: lmbFlash
    ? "2px solid #7ecc49"
    : rmbFlash
    ? "2px solid #ffcc44"
...
export default function AnkaRecorder({ bot }: { bot: BotState }) {
  ...
  // Track which slots flashed and with which button
  ...
  useEffect(() => {
    ...
    const unsubInv = window.electronAPI.on("bot:inventoryUpdated", (d: { botId: string; inventory: WindowSlot[] }) => {
      if (d.botId === bot.id) setBotInventory(d.inventory);
    });

    return () => { unsubWindow(); unsubClose(); unsubInv(); };
  }, [bot.id]);
  ...
  const flashSlot = useCallback((slot: number, button: number) => {
    if (button === 0) {
      setLmbFlash(prev => new Set([...prev, slot]));
      setTimeout(() => setLmbFlash(prev => { const s = new Set(prev); s.delete(slot); return s; }), 350);
    } else {
      setRmbFlash(prev => new Set([...prev, slot]));
      setTimeout(() => setRmbFlash(prev => { const s = new Set(prev); s.delete(slot); return s; }), 350);
    }
  }, []);

  // Main handler: live-click on bot AND record step if recording
  const handleSlotClick = useCallback(async (slot: WindowSlot, button: number, windowTitle: string) => {
    ...
    // 1. Execute live on bot
    await window.electronAPI.anka.clickSlot(bot.id, slot.slot, button).catch(() => {});

    // 2. If recording — also add step
    if (recording) {
  ...
  function onSlotLMB(slot: WindowSlot, windowTitle: string) {
    handleSlotClick(slot, 0, windowTitle);
  }

  function onSlotRMB(e: React.MouseEvent, slot: WindowSlot, windowTitle: string) {
    e.preventDefault();
  ...
  async function playProfile(profile: AnkaProfile) {
    ...
    const result = await window.electronAPI.anka.play(bot.id, profile.id)
      .catch((e: any) => ({ error: e.message }));
    setPlayMsg((result as any).error ? `❌ ${(result as any).error}` : "✅ Выполнено!");
  ...
  // Build a full 54-slot grid for chest, or 36-slot for inventory
  function buildGrid(slots: WindowSlot[], cols = 9): WindowSlot[] {
    const maxSlot = slots.reduce((m, s) => Math.max(m, s.slot), -1);
    const size = Math.max(maxSlot + 1, cols);
    const grid: WindowSlot[] = Array.from({ length: size }, (_, i) => ({
      slot: i, name: "", displayName: "", count: 0,
    }));
    for (const s of slots) grid[s.slot] = s;
    return grid;
  }

  function SlotGrid({ slots, windowTitle, cols = 9, label }: {
    slots: WindowSlot[];
    windowTitle: string;
    cols?: number;
    label?: string;
      ...
      <div>
        ...
        <div style={{
          ...
          {grid.map((slot) => (
            <div
              key={slot.slot}
              onClick={() => onSlotLMB(slot, windowTitle)}
              onContextMenu={(e) => onSlotRMB(e, slot, windowTitle)}
              title={slot.name
                ? `Слот ${slot.slot}: ${slot.displayName || slot.name}${slot.count > 1 ? ` x${slot.count}` : ""}\nЛКМ = взять в руку / выбрать\nПКМ = правый клик`
                : `Слот ${slot.slot} (пустой)`}
              style={slotStyle(
                !slot.name,
                lmbFlash.has(slot.slot),
                rmbFlash.has(slot.slot),
                windowTitle === "__inventory__" && slot.slot <= 8,
              )}
            >
              {slot.name ? (
                <>
  ...
  return (
    <div style={{ fontFamily: "monospace" }}>
      ...
      <div className="p-3 flex flex-col gap-3">
        ...
        {/* CHEST WINDOW (when bot has open chest/menu) */}
        {currentWindow ? (
          <div style={{ background: "#0f1a0f", border: "1px solid #2a4a2a", borderRadius: 8, padding: 10 }}>
            <div className="flex items-center gap-2 mb-2">
              ...
              {recording && (
                <span className="text-xs" style={{ color: "#ff5555" }}>● запись</span>
              )}
            </div>
            ...
            <div className="text-xs mt-2" style={{ color: "#444" }}>
              Кликай по предметам — бот повторит клик в реальном времени
            </div>
          </div>
        ) : isOnline ? (
        ...
        {/* BOT INVENTORY (always shown when online) */}
          ...
          <div style={{ background: "#0f0f1a", border: "1px solid #2a2a4a", borderRadius: 8, padding: 10 }}>
            ...
            <div style={{ overflowX: "auto" }}>
              <SlotGrid
                ...
                cols={GRID_COLS}
                label="Хотбар (строка 1) → инвентарь"
              />
            </div>
            <div className="text-xs mt-2" style={{ color: "#444" }}>
              ЛКМ = взять предмет в руку · ПКМ = правый клик по слоту
            </div>
          </div>
        )}
                ...
                <div key={p.id} style={{
                  ...
                  <div className="flex items-start justify-between">
                    <div className="flex-1 min-w-0">
                      ...
                      {/* Steps preview with LMB/RMB indicators */}
